...
from Test.kernels import laplace_kernel, kernel_midwidth_lap

"""
Module containing statistical independence tests with learnable Laplace kernels.
"""

__author__ = 'ryx'

class IndpTest_LKLaplace(IndpTest):
    """
    ...
    def __init__(self, X, Y, device, alpha=0.05, n_permutation=100, null_gamma = True, split_ratio = 0.5):
        ...
        """
        super(IndpTest_LKLaplace, self).__init__(X, Y, alpha)
    ...
    def perform_test(self, if_grid_search = False, debug = -1):
        ...
        if if_grid_search:
            wx_mid, wy_mid, wx_max, wy_max = self.midwidth_lap(Xtr, Ytr)
            wx_init, wy_init = self.grid_search_init(Xtr, Ytr, wx_mid, wy_mid)
        else:
            wx_init, wy_init, wx_max, wy_max = self.midwidth_lap(Xtr, Ytr)
            
        Xtr = Xtr.to(self.device)    
        Ytr = Ytr.to(self.device)
        
        wx, wy, path = self.search_width(Xtr, Ytr, wx_init, wy_init, wx_max, wy_max, debug = debug) 
