...
"""
Module containing statistical independence tests with selecting Gaussian/Laplace kernels (learnable width parameters).
"""
__author__ = 'ryx'

class IndpTest_LKSelect_GauLap(IndpTest):
    """
    ...
    def __init__(self, X, Y, device, alpha=0.05, n_permutation=100, null_gamma = True, split_ratio = 0.5):
        ...
        """
        super(IndpTest_LKSelect_GauLap, self).__init__(X, Y, alpha)
    ...
    def perform_test(self, if_grid_search = False, debug = -1):
        ...
        wx_init_rbf, wy_init_rbf, _, _ = self.midwidth_rbf(Xtr, Ytr)
        wx_init_lap, wy_init_lap, _, _ = self.midwidth_lap(Xtr, Ytr)
            
        Xtr = Xtr.to(self.device)    
        Ytr = Ytr.to(self.device)
        
        wx_rbf, wy_rbf, wx_lap, wy_lap, b_list = self.search_width_weight_selection(Xtr, Ytr, wx_init_rbf, wy_init_rbf, wx_init_lap, wy_init_lap, debug = debug) 
        
        if self.null_gamma == True:
            K, L = self.cal_kernels(Xte, Yte, wx_rbf, wy_rbf, wx_lap, wy_lap, b_list)
            Kc = K - torch.mean(K,0)
            Lc = L - torch.mean(L,1)
            ...
            Lc = L - torch.mean(L,1)
            testStat,_ = self.J_maxpower_term(K, L, Kc, Lc)
            thresh = self.cal_thresh_pm(Xte, Yte, wx_rbf, wy_rbf, wx_lap, wy_lap, b_list)  
    ...
    def search_width_weight_selection(self, X, Y, wx_init_rbf, wy_init_rbf, wx_init_lap, wy_init_lap, lr = 0.05, \
                            delta_estimate_grad=1e-3, iter_steps = 100, debug = -1):

        wx_log_init_rbf = torch.log(wx_init_rbf)
        wy_log_init_rbf = torch.log(wy_init_rbf)
        wx_log_init_lap = torch.log(wx_init_lap)
        wy_log_init_lap = torch.log(wy_init_lap)
        ...
        if use_gpu:
            wx_log_rbf = torch.tensor([wx_log_init_rbf],requires_grad=True, device = device)
            wy_log_rbf = torch.tensor([wy_log_init_rbf],requires_grad=True, device = device)
            wx_log_lap = torch.tensor([wx_log_init_lap],requires_grad=True, device = device)
            wy_log_lap = torch.tensor([wy_log_init_lap],requires_grad=True, device = device)
            beta = torch.tensor([0., 0., 0., 0.], requires_grad=True, device = device)
        else:
            wx_log_rbf = torch.tensor([wx_log_init_rbf],requires_grad=True)
            wy_log_rbf = torch.tensor([wy_log_init_rbf],requires_grad=True)
            wx_log_lap = torch.tensor([wx_log_init_lap],requires_grad=True)
            wy_log_lap = torch.tensor([wy_log_init_lap],requires_grad=True)
            beta = torch.tensor([0., 0., 0., 0.], requires_grad=True)

        optimizer = optim.Adam([wx_log_rbf, wy_log_rbf, wx_log_lap, wy_log_lap, beta], lr=lr)
        ...
        for st in range(iter_steps):
            ...
            wx_rbf = torch.exp(wx_log_rbf)
            wy_rbf = torch.exp(wy_log_rbf)
            wx_lap = torch.exp(wx_log_lap)
            wy_lap = torch.exp(wy_log_lap)

            K_rbf = rbf_kernel(X, X, wx_rbf)
            L_rbf = rbf_kernel(Y, Y, wy_rbf)
            K_lap = laplace_kernel(X, X, wx_lap)
            L_lap = laplace_kernel(Y, Y, wy_lap)

            weight = torch.exp(beta)
            b0 = weight[0]/torch.sum(weight[:2])
            b1 = weight[1]/torch.sum(weight[:2])
            b2 = weight[2]/torch.sum(weight[2:])
            b3 = weight[3]/torch.sum(weight[2:])
            ...
            if debug > 0:
                if st%debug==0:
                    ...
                    print("width", (wx_rbf.item(), wy_rbf.item(), wx_lap.item(), wy_lap.item()), testStat.item())
                    print("beta", [b0.item(),b1.item(),b2.item(),b3.item()])
                    
            optimizer.step()

        return wx_rbf.item(),wy_rbf.item(),wx_lap.item(), wy_lap.item(), [b0.item(),b1.item(),b2.item(),b3.item()]
    ...
    def cal_kernels(self, X, Y, wx_rbf, wy_rbf, wx_lap, wy_lap, b_list):
        """
        Calculate kernels
        """
        K_rbf = rbf_kernel(X, X, wx_rbf)
    ...
    def cal_thresh_pm(self, X, Y, wx_rbf, wy_rbf, wx_lap, wy_lap, b_list):
        ind = []
        for _ in range(self.n_permutation):
            p = np.random.permutation(len(X))
            Xp = X[p]
            K, L = self.cal_kernels(Xp, Y, wx_rbf, wy_rbf, wx_lap, wy_lap, b_list)
            Kc = K - torch.mean(K,0)
            Lc = L - torch.mean(L,1)
