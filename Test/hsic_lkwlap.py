...
from Test.kernels import lap_kernel_weight, kernel_midwidth_lap

class IndpTest_LKWeightLaplace(IndpTest):
    """
    Independence test with learnable weighted Laplace kernels (width/weight parameters).
    ...
    def __init__(self, X, Y, device, alpha=0.05, n_permutation=100, null_gamma = True, split_ratio = 0.5):
        ...
        """
        super(IndpTest_LKWeightLaplace, self).__init__(X, Y, alpha)
    ...
    def perform_test(self, debug = -1):
        ...
        wx_mid, wy_mid, _, _= self.midwidth_lap(Xtr, Ytr)
    ...
    def cal_weight_kernels(self, X, Y, kernel_width_x, kernel_width_y, weight_x, weight_y):
        """
        Calculate (weighted lap) kernels
        """
        K = lap_kernel_weight(X, X, kernel_width_x, weight_x)
        L = lap_kernel_weight(Y, Y, kernel_width_y, weight_y)
